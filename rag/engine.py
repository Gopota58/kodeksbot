...
class RAGEngine:
    ...
    def _retrieve_hybrid_multi(self, queries: list, k: int, hyde_vector=None):
        ...
        if hyde_vector is not None:
            try:
                hv_res = self.vectorstore.similarity_search_with_score_by_vector(hyde_vector, k=top_n)
        ...
        if kw_hits == 0:
            return []
        # Релевантность решаем по ИСХОДНОМУ запросу, а не по LLM-переформулировке.
        # Иначе rewrite «спасает» нерелевантный вопрос: напр. для «пенсия» он находит
        # «пенсионное обеспечение» в ТК/НК, хотя возраст выхода на пенсию в 26 кодексах
        # отсутствует, — и модель галлюцинирует (ФЗ-400, чужие кодексы). Если по самому
        # вопросу ближайший чанк слабо похож (выше порога) — контекста нет, возвращаем
        # пусто, модель честно отвечает «нет ответа».
        if orig_best is None or orig_best > self._MAX_IRRELEVANT_DISTANCE:
            return []
    ...
    def ask_with_sources(self, question: str, max_retries: int = 3):
        ...
        if self._is_no_answer(answer):
            # Санитайзер: даже если модель добавила «знания из памяти» к фразе «нет ответа»,
            # отдаём ТОЛЬКО каноническую фразу и пустые источники (защита от галлюцинаций).
            return CANONICAL_NO_ANSWER, []
        sources = [
            {
