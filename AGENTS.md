...
- `bot.py` / `desktop_client.py` — Telegram-бот / десктоп (опц., из фреймворка). Бот для `/ask`
  использует публичный ключ, для `/reindex` — обязательно админский (`resolved_admin_api_key`),
  иначе 401
...
- **Ребрендинг и смена модели эмбеддингов расползаются по файлам, которые никто не проверяет:**
  `pyproject.toml` (name/description/keywords), `bot.py`, `desktop_client.py`, `start.sh`,
  `download_model.py`, комментарии и тексты ошибок в `config.py`. После любой такой смены
  гонять grep по `kotobaza|Котобаза|rubert` по всему репозиторию — иначе публичная репа
  будет описывать проект, которого уже нет.
- **`download_model.py` не должен импортировать `config` до скачивания:** `config` выставляет
  `HF_HUB_OFFLINE=1`, и `snapshot_download` падает. Offline-флаги снимаются внутри скрипта
  перед явным скачиванием.
- **`models/` и `data/docs/` не в репозитории** (`.gitignore`). Любая инструкция «быстрый старт»
  обязана содержать шаг получения весов, иначе свежий клон падает на `ingest.py`.
- **Адрес живого демо не публиковать** в файлах и README: стенд на бесплатном гранте, а `/ask`
  упирается в квоту GigaChat. `gh` CLI на машине не установлен — метаданные репозитория
  (description/topics) меняются руками на странице GitHub.
